<script type="text/javascript">
    MC_AJAX_CALL = "{$url_base}modules/popupaddtocart/popupaddtocart_ajax.php?ajax=1";
    ID_LANG = {$id_lang};
...
{if isset($accessories) AND $accessories}
    ...
    <div class="modal-crossselling">
        <div style="background: #000000; opacity: 0.8;" class="ppopup_close ppopup_overlay">
        </div>
        ...
        <div style="
            ...
            <div class=product-title-main-poup>
                <h3>
                    {l s='We thought you may also be interested in these accessories:'}
                </h3>
            </div>
            <div class="main-poup">
                <a class="close">&nbsp;</a>
                    ...
                    <div class="pcol index_{($smarty.foreach.accessories_list.index + 1)}">
                        <form action="{$url_base}index.php?controller=cart" method="post">
                            <div class="ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
                                {include file="$tpl_dir/../../modules/popupaddtocart/createdjs.tpl"}
                            ...
                            <p id="minimal_quantity_wanted_p"{if $accessory.product->minimal_quantity <= 1 OR !$accessory.product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
                                {l s='This product is not sold individually. You must select at least'} <b id="minimal_quantity_label">{$accessory.product->minimal_quantity}</b> {l s='quantity for this product.'}
                            </p>
                            ...
                            <p id="availability_statut"{if ($accessory.product->quantity <= 0 && !$accessory.product->available_later && $accessory.allow_oosp) OR ($accessory.product->quantity > 0 && !$accessory.product->available_now) OR !$accessory.product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
                                ...
                                <span id="availability_value"{if $accessory.product->quantity <= 0} class="warning_inline"{/if}>{if $accessory.product->quantity <= 0}{if $accessory.allow_oosp}{$accessory.product->available_later}{else}{l s='This product is no longer in stock'}{/if}{else}{$accessory.product->available_now}{/if}</span>				
                            </p>
                            ...
                            {if ($accessory.display_qties == 1 && !$PS_CATALOG_MODE && $accessory.product->available_for_order)}
                                <p id="pQuantityAvailable"{if $accessory.product->quantity <= 0} style="display: none;"{/if}>
                                    ...
                                    <span {if $accessory.product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s='Items in stock'}</span>
                                </p>
                            {/if}
                            <!-- Out of stock hook -->
                            ...
                            <p class="warning_inline" id="last_quantities"{if ($accessory.product->quantity > $accessory.last_qties OR $accessory.product->quantity <= 0) OR $accessory.allow_oosp OR !$accessory.product->available_for_order OR $PS_CATALOG_MODE} style="display: none"{/if} >{l s='Warning: Last items in stock!'}</p>
                            ...
                            {if $priceDisplay == 2}
                                <br />
                                <span id="pretaxe_price"><span id="pretaxe_price_display">{convertPrice price=$accessory.product->getPrice(false, $smarty.const.NULL)}</span>&nbsp;{l s='tax excl.'}</span>
                            {/if}
                        </div>
                        ...
                        <p id="old_price"{if !$accessory.product->specificPrice || !$accessory.product->specificPrice.reduction} class="hidden"{/if}>
                            {if $priceDisplay >= 0 && $priceDisplay <= 2}
                                ...
                                <!-- {if $accessory.tax_enabled && $display_tax_label == 1}{if $priceDisplay == 1}{l s='tax excl.'}{else}{l s='tax incl.'}{/if}{/if} -->
                            {/if}
                        </p>
                        {if $accessory.packItems|@count && $productPrice < $accessory.product->getNoPackPrice()}
                            <p class="pack_price">{l s='Instead of'} <span style="text-decoration: line-through;">{convertPrice price=$accessory.product->getNoPackPrice()}</span></p>
                            <br class="clear" />
                        {/if}
                        {if $accessory.product->ecotax != 0}
                            <p class="price-ecotax">{l s='Include'} <span id="ecotax_price_display">{if $priceDisplay == 2}{$accessory.ecotax_tax_exc|convertAndFormatPrice}{else}{$accessory.ecotax_tax_inc|convertAndFormatPrice}{/if}</span> {l s='For green tax'}
                        ...
                        {if !empty($accessory.product->unity) && $accessory.product->unit_price_ratio > 0.000000}
                            ...
                            <p class="unit-price"><span id="unit_price_display">{convertPrice price=$unit_price}</span> {l s='per'} {$accessory.product->unity|escape:'htmlall':'UTF-8'}</p>
                        {/if}
                        ...
                        {/if}
                            <p id="padd_to_cart" {if (!$accessory.allow_oosp && $accessory.product->quantity <= 0) OR !$accessory.product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE}style="display:none"{/if} class="pbuttons_bottom_block">
                                <span></span>
                                <input type="submit" name="Submit" value="{l s='Add to cart'}" class="pexclusive" />
                            </p>
            ...
            {if ($smarty.foreach.accessories_list.index + 1) % 3==0 AND $smarty.foreach.accessories_list.index > 0}
                <div class="clear"></div>
            {/if}
            {/foreach}
            </div>
            <div class="pfooter">
                ...
                <div>
                    <a href="{$link->getPageLink("order-opc", true)|escape:'html'}" id="button_order_cart" class="exclusive_large" title="{l s='Check out' mod='blockcart'}" rel="nofollow"><span></span>{l s='Check out' mod='blockcart'}</a>
                </div>
            </div>
        </div>
    </div>
