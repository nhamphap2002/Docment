...
/*
 Created on : Feb 13, 2014, 3:53:33 PM
...
//     $('.modal-crossselling .pinner ').jScrollPane();
}
...
$(document).ready(function() {
    ...
    $('.ppopup_close, #pcontinueshop,.close').live('click', function(e) {
        e.preventDefault()
    ...
    //add scroll
// $('.modal-crossselling .pinner ').jScrollPane();
//var po_top=window.outerWidth();
//var po_left=window.outerHeight();
//var popup_left=(po_top - 720)/2;
//var popup_top=(po_top- 530)/2
//$('.p_popin').css({
//    'top':popup_top+'px',
//    'left':popup_left+'px'
//})
...
function  loadData(pform) {
    ...
    var p_pid_product = '';
    p_pid_product = 'p_' + pid_product;

    if (typeof arrproductHasAttributes[p_pid_product] != 'undefined')
        pproductHasAttributes = arrproductHasAttributes[p_pid_product];

    if (typeof arrquantitiesDisplayAllowed[p_pid_product] != 'undefined')
        pquantitiesDisplayAllowed = arrquantitiesDisplayAllowed[p_pid_product];

    if (typeof arrquantityAvailable[p_pid_product] != 'undefined')
        pquantityAvailable = parseInt(arrquantityAvailable[p_pid_product]);

    if (typeof arrallowBuyWhenOutOfStock[p_pid_product] != 'undefined')
        pallowBuyWhenOutOfStock = arrallowBuyWhenOutOfStock[p_pid_product];

    if (typeof arravailableNowValue[p_pid_product] != 'undefined')
        pavailableNowValue = arravailableNowValue[p_pid_product];

    if (typeof arravailableLaterValue[p_pid_product] != 'undefined')
        pavailableLaterValue = arravailableLaterValue[p_pid_product];

    if (typeof arrproductPriceTaxExcluded[p_pid_product] != 'undefined')
        pproductPriceTaxExcluded = parseFloat(arrproductPriceTaxExcluded[p_pid_product]);

    if (typeof arrproductBasePriceTaxExcluded[p_pid_product] != 'undefined')
        pproductBasePriceTaxExcluded = parseFloat(arrproductBasePriceTaxExcluded[p_pid_product]);

    if (typeof arrreduction_percent[p_pid_product] != 'undefined')
        preduction_percent = parseInt(arrreduction_percent[p_pid_product]);

    if (typeof arrreduction_price[p_pid_product] != 'undefined')
        preduction_price = parseInt(arrreduction_price[p_pid_product]);

    if (typeof arrspecific_price[p_pid_product] != 'undefined')
        pspecific_price = parseFloat(arrspecific_price[p_pid_product]);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_specific_price'] = parseInt(arrproduct_specific_price[p_pid_product]['id_specific_price']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_specific_price_rule'] = parseInt(arrproduct_specific_price[p_pid_product]['id_specific_price_rule']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_cart'] = parseInt(arrproduct_specific_price[p_pid_product]['id_cart']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_product'] = parseInt(arrproduct_specific_price[p_pid_product]['id_product']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_shop'] = parseInt(arrproduct_specific_price[p_pid_product]['id_shop']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_shop_group'] = parseInt(arrproduct_specific_price[p_pid_product]['id_shop_group']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_currency'] = parseInt(arrproduct_specific_price[p_pid_product]['id_currency']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_country'] = parseInt(arrproduct_specific_price[p_pid_product]['id_country']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_group'] = parseInt(arrproduct_specific_price[p_pid_product]['id_group']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_customer'] = parseInt(arrproduct_specific_price[p_pid_product]['id_customer']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['id_product_attribute'] = parseInt(arrproduct_specific_price[p_pid_product]['id_product_attribute']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['price'] = parseFloat(arrproduct_specific_price[p_pid_product]['price']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['from_quantity'] = parseInt(arrproduct_specific_price[p_pid_product]['from_quantity']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['reduction'] = parseFloat(arrproduct_specific_price[p_pid_product]['reduction']);

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['reduction_type'] = arrproduct_specific_price[p_pid_product]['reduction_type'];

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['from'] = arrproduct_specific_price[p_pid_product]['from'];

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['to'] = arrproduct_specific_price[p_pid_product]['to'];

    if (typeof arrproduct_specific_price[p_pid_product] != 'undefined')
        pproduct_specific_price['score'] = parseInt(arrproduct_specific_price[p_pid_product]['score']);

    if (typeof arrspecific_currency[p_pid_product] != 'undefined')
        pspecific_currency = arrspecific_currency[p_pid_product];

    if (typeof arrgroup_reduction[p_pid_product] != 'undefined')
        pgroup_reduction = parseInt(arrgroup_reduction[p_pid_product]);

    if (typeof arrdefault_eco_tax[p_pid_product] != 'undefined')
        pdefault_eco_tax = parseFloat(arrdefault_eco_tax[p_pid_product]);

    if (typeof arrecotaxTax_rate[p_pid_product] != 'undefined')
        pecotaxTax_rate = parseInt(arrecotaxTax_rate[p_pid_product]);

    if (typeof arrcurrentDate[p_pid_product] != 'undefined')
        pcurrentDate = arrcurrentDate[p_pid_product];

    if (typeof arrmaxQuantityToAllowDisplayOfLastQuantityMessage[p_pid_product] != 'undefined')
        pmaxQuantityToAllowDisplayOfLastQuantityMessage = parseInt(arrmaxQuantityToAllowDisplayOfLastQuantityMessage[p_pid_product]);

    if (typeof arrnoTaxForThisProduct[p_pid_product] != 'undefined')
        pnoTaxForThisProduct = arrnoTaxForThisProduct[p_pid_product];

    if (typeof arrdisplayPrice[p_pid_product] != 'undefined')
        pdisplayPrice = parseInt(arrdisplayPrice[p_pid_product]);

    if (typeof arrproductReference[p_pid_product] != 'undefined')
        pproductReference = arrproductReference[p_pid_product];

    if (typeof arrproductAvailableForOrder[p_pid_product] != 'undefined')
        pproductAvailableForOrder = parseInt(arrproductAvailableForOrder[p_pid_product]);

    if (typeof arrproductShowPrice[p_pid_product] != 'undefined')
        pproductShowPrice = parseInt(arrproductShowPrice[p_pid_product]);

    if (typeof arrproductUnitPriceRatio[p_pid_product] != 'undefined')
        pproductUnitPriceRatio = parseFloat(arrproductUnitPriceRatio[p_pid_product]);

    if (typeof arridDefaultImage[p_pid_product] != 'undefined')
        pidDefaultImage = parseInt(arridDefaultImage[p_pid_product]);

    if (typeof arrstock_management[p_pid_product] != 'undefined')
        pstock_management = parseInt(arrstock_management[p_pid_product]);

    if (typeof arrproductPriceWithoutReduction[p_pid_product] != 'undefined')
        pproductPriceWithoutReduction = parseFloat(arrproductPriceWithoutReduction[p_pid_product]);

    if (typeof arrproductPrice[p_pid_product] != 'undefined')
        pproductPrice = parseFloat(arrproductPrice[p_pid_product]);
}